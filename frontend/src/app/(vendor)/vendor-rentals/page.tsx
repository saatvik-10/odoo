  ...
  import { Rental } from '@/validators/rental.validator';
  ...
  export default function RentalDashboard() {
    ...
    const handleRoute = (order: Rental) => {
    ...
    const filteredOrders = rentals?.filter((order) => {
      const matchesStatus =
        ...
        (activeStatusFilter === 'Pickup' && order.status === 'Picked Up') ||
        (activeStatusFilter === 'Returned' && order.status === 'Returned');

      const matchesSearch =
        searchQuery === '' ||
        order.user.id.toLowerCase().includes(searchQuery.toLowerCase()) ||
